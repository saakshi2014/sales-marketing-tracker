...
async function loadChangeRequests() {
    ...
    const badgeEl   = document.getElementById('cr-pending-badge');
    ...
    try {
        const res      = await fetch('/api/leads/change-requests/pending');
        ...
        requests.forEach(r => {
            const fieldLabel =
                r.field === 'email'       ? 'Email Address' :
                r.field === 'phone'       ? 'Phone Number'  :
                r.field === 'linkedinUrl' ? 'LinkedIn URL'  : r.field;
...
async function reviewRequest(requestId, action) {
    const msg = action === 'approve'
        ? 'Approve this contact change?'
        : 'Reject this contact change?';
    if (!confirm(msg)) return;

    try {
        const res = await fetch(
            `/api/leads/change-request/${requestId}/review`,
            {
                method:  'POST',
        ...
        if (res.ok) {
            showToast(
                action === 'approve'
                    ? '✓ Change approved and applied to lead.'
